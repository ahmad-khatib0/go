...
import (
	...
	"github.com/ahmad-khatib0/go/test-driven-development/ch04_test_suites/handlers"
...
func TestUpsertUser_Load(t *testing.T) {
	...
	for i := 0; i < LOAD_AMOUNT; i++ {
		t.Run("concurrent upsert", func(t *testing.T) {
			...
			// close the body of the response in order to allow the same connection
			// to be reused by another goroutine
			r.Body.Close()
			require.Nil(t, err)
