...
func init() {
	...
	isLE = (xb[0] == 0x00) // check what the first byte is to determine the value of isLE
}

func DataFromBytesUnsafe(b [16]byte) Data {
	data := *(*Data)(unsafe.Pointer(&b))
	if isLE { //  check our flag to see if we are on a little-endian platform
...
// write our Data back to the network
