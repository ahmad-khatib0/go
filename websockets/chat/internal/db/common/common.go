...
func SelectEarliestUpdatedSubs(subs []types.Subscription, opts *types.QueryOpt, maxResults int) []types.Subscription {
	limit := maxResults
	ims := time.Time{}
	if opts != nil {
	...
	// Now that we fetched potentially more subscriptions than needed,
	// we got to take those with the oldest modifications.
