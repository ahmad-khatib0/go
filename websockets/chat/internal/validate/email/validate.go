...
type validator struct {
	Cfg *config.ValidatorConfigEmail
...
func (v *validator) Init(conf any, store *store.Store) error {
	cfg, ok := conf.(*config.ValidatorConfigEmail)
	if !ok {
		return fmt.Errorf("failed to cast config")
	}
	v.Cfg = cfg

	sender, err := mail.ParseAddress(v.Cfg.Sender)
	if err != nil {
		return err
	}
	...
	if v.Cfg.Login != "" {
		mechanism := strings.ToLower(v.Cfg.AuthMechanism)
		switch mechanism {
		case "cram-md5":
			v.auth = smtp.CRAMMD5Auth(v.Cfg.Login, v.Cfg.Password)
		case "login":
			v.auth = &loginAuth{[]byte(v.Cfg.Login), []byte(v.Cfg.Password)}
		case "", "plain":
			v.auth = smtp.PlainAuth("", v.Cfg.Login, v.Cfg.Password, v.Cfg.SmtpServer)
		default:
	...
	v.Cfg.ValidationTemplFile, err = validate.ResolveTemplatePath(v.Cfg.ValidationTemplFile)
	if err != nil {
		return err
	}

	v.Cfg.ResetTemplFile, err = validate.ResolveTemplatePath(v.Cfg.ResetTemplFile)
	if err != nil {
		return err
	}
	...
	validationPathTempl, err = textt.New("validation").Parse(v.Cfg.ValidationTemplFile)
	if err != nil {
		return err
	}
	resetPathTempl, err = textt.New("reset").Parse(v.Cfg.ResetTemplFile)
	if err != nil {
		return err
	}

	var path string
	if len(v.Cfg.Languages) > 0 {
		v.validationTempl = make([]*textt.Template, len(v.Cfg.Languages))
		v.resetTempl = make([]*textt.Template, len(v.Cfg.Languages))
		var langTags []i18n.Tag
		// Find actual content templates for each defined language.
		for idx, lang := range v.Cfg.Languages {
	...
	if v.Cfg.HostUrl, err = validate.ValidateHostURL(v.Cfg.HostUrl); err != nil {
		return err
	}

	if v.Cfg.SmtpHeloHost == "" {
		hostUrl, _ := url.Parse(v.Cfg.HostUrl)
		v.Cfg.SmtpHeloHost = hostUrl.Hostname()
	}
	if v.Cfg.SmtpHeloHost == "" {
	...
	if v.Cfg.MaxRetries == 0 {
		v.Cfg.MaxRetries = defaultMaxRetries
	}

	if v.Cfg.CodeLength == 0 {
		v.Cfg.CodeLength = defaultCodeLength
	}
	v.maxCodeValue = big.NewInt(0).Exp(big.NewInt(10), big.NewInt(int64(v.Cfg.CodeLength)), nil)

	if v.Cfg.SmtpPort == "" {
		v.Cfg.SmtpPort = defaultPort
	}
...
func (v *validator) PreCheck(cred string, _ map[string]interface{}) (string, error) {
	...
	if len(v.Cfg.Domains) > 0 {
		...
		var found bool
		for _, domain := range v.Cfg.Domains {
			if domain == parts[1] {
				found = true
				break
			}
...
func (v *validator) Request(user t.Uid, email, lang, resp string, tmpToken []byte) (bool, error) {
	...
	resp = strings.Repeat("0", v.Cfg.CodeLength-len(resp)) + resp
	...
	content, err := validate.ExecuteTemplate(
		template,
		templateParts,
		map[string]interface{}{
			"Token":   url.QueryEscape(string(token)),
			"Code":    resp,
			"HostUrl": v.Cfg.HostUrl})
	if err != nil {
		return false, err
	}

	// Create or update validation record in DB.
	isNew, err := store.Users.UpsertCred(
		&t.Credential{
			User:   user.String(),
...
func (v *validator) Check(user t.Uid, resp string) (string, error) {
	...
	if cred.Retries > v.Cfg.MaxRetries {
		return "", t.ErrPolicy
	}
...
func (v *validator) sendMail(rcpt []string, msg []byte) error {
	client, err := smtp.Dial(v.SMTPAddr + ":" + v.Cfg.SmtpPort)
	if err != nil {
		return err
	}
	defer client.Close()
	if err = client.Hello(v.Cfg.SmtpHeloHost); err != nil {
		return err
	}
