...
func NewApiKey(l *logger.Logger, salt []byte) *ApiKey {
	return &ApiKey{logs: l, salt: salt}
}
...
func (ak *ApiKey) CheckAPIKey(apikey string) (isValid, isRoot bool) {
	...
	if err != nil {
		ak.logs.Sugar().Warnf("failed to decode.base64 appid  %v", err)
		return
	}

	if data[0] != 1 {
		ak.logs.Sugar().Warnf("unknown appid signature algorithm ", data[0])
		return
	}

	hasher := hmac.New(md5.New, ak.salt)
	...
	if !bytes.Equal(data[apikeyVersion+apikeyAppID+apikeySequence+apikeyWho:], check) {
		ak.logs.Warn("invalid apikey signature")
		return
	}
