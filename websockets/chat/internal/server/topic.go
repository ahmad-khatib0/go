...
func (t *Topic) registerSession(msg *ClientComMessage) {
	...
	if t.isInactive() {
		...
		if err := t.handleSubscription(msg); err == nil {
			...
			globals.l.Sugar().Warnf("topic[%s] subscription failed %v, sid=%s", t.name, err, msg.sess.sid)
		}
	}
...
func (t *Topic) handleMetaSet(msg *ClientComMessage, asUid types.Uid, asChan bool, authLevel auth.Level) {
	if msg.MetaWhat&constMsgMetaDesc != 0 {
		if err := t.replySetDesc(msg.sess, asUid, asChan, authLevel, msg); err == nil {
			...
			globals.l.Sugar().Warnf("topic[%s] meta.Set.Desc failed: %v", t.name, err)
		}
	}
	if msg.MetaWhat&constMsgMetaSub != 0 {
		if err := t.replySetSub(msg.sess, msg, asChan); err != nil {
			globals.l.Sugar().Warnf("topic[%s] meta.Set.Sub failed: %v", t.name, err)
		}
	}
	if msg.MetaWhat&constMsgMetaTags != 0 {
		if err := t.replySetTags(msg.sess, asUid, msg); err != nil {
			globals.l.Sugar().Warnf("topic[%s] meta.Set.Tags failed: %v", t.name, err)
		}
	}
	if msg.MetaWhat&constMsgMetaCred != 0 {
		if err := t.replySetCred(msg.sess, asUid, authLevel, msg); err != nil {
			globals.l.Sugar().Warnf("topic[%s] meta.Set.Cred failed: %v", t.name, err)
		}
	}
}
...
func (t *Topic) handleMetaDel(msg *ClientComMessage, asUid types.Uid, asChan bool, authLevel auth.Level) {
	...
	if err != nil {
		globals.l.Sugar().Warnf("topic[%s] meta.Del failed: %v", t.name, err)
	}
}
...
func (t *Topic) handleSessionUpdate(upd *sessionUpdate, currentUA *string, uaTimer *time.Timer) {
	if upd.sess != nil {
		...
		if t.cat != types.TopicCatMe {
			globals.l.Sugar().Panicf("invalid topic category in UA update", t.name)
		}
...
func (t *Topic) handleClientMsg(msg *ClientComMessage) {
	if msg.Pub != nil {
		...
		globals.l.Sugar().Panicf("topic: wrong client message type for broadcasting", t.name)
	}
}
...
func (t *Topic) handleServerMsg(msg *ServerComMessage) {
	...
	if msg.Pres != nil {
		...
		// TODO(gene): maybe remove this panic.
		globals.l.Sugar().Panicf("topic: wrong server message type for broadcasting", t.name)
	}
}
...
func (t *Topic) handleSubscription(msg *ClientComMessage) error {
	...
	if getWhat&constMsgMetaDesc != 0 {
		...
		if err := t.replyGetDesc(msg.sess, asUid, asChan, msgsub.Get.Desc, msg); err != nil {
			globals.l.Sugar().Warnf("topic[%s] handleSubscription Get.Desc failed: %v sid=%s", t.name, err, msg.sess.sid)
		}
	}
	...
	if getWhat&constMsgMetaSub != 0 {
		...
		if err := t.replyGetSub(msg.sess, asUid, authLevel, asChan, msg); err != nil {
			globals.l.Sugar().Warnf("topic[%s] handleSubscription Get.Sub failed: %v sid=%s", t.name, err, msg.sess.sid)
		}
	}
	...
	if getWhat&constMsgMetaTags != 0 {
		...
		if err := t.replyGetTags(msg.sess, asUid, msg); err != nil {
			globals.l.Sugar().Warnf("topic[%s] handleSubscription Get.Tags failed: %v sid=%s", t.name, err, msg.sess.sid)
		}
	}
	...
	if getWhat&constMsgMetaCred != 0 {
		// Send get.tags response as a separate {meta} packet
		if err := t.replyGetCreds(msg.sess, asUid, msg); err != nil {
			globals.l.Sugar().Warnf("topic[%s] handleSubscription Get.Cred failed: %v sid=%s", t.name, err, msg.sess.sid)
		}
	}
	...
	if getWhat&constMsgMetaData != 0 {
		...
		if err := t.replyGetData(msg.sess, asUid, asChan, msgsub.Get.Data, msg); err != nil {
			globals.l.Sugar().Warnf("topic[%s] handleSubscription Get.Data failed: %v sid=%s", t.name, err, msg.sess.sid)
		}
	}
	...
	if getWhat&constMsgMetaDel != 0 {
		...
		if err := t.replyGetDel(msg.sess, asUid, msgsub.Get.Del, msg); err != nil {
			globals.l.Sugar().Warnf("topic[%s] handleSubscription Get.Del failed: %v sid=%s", t.name, err, msg.sess.sid)
		}
	}
	...
	case types.TopicCatMe:
		...
		if !t.isLoaded() {
			...
			if err := t.loadContacts(asUid); err != nil {
				globals.l.Sugar().Errorf("topic: failed to load contacts", t.name, err.Error())
			}
...
func (t *Topic) handlePubBroadcast(msg *ClientComMessage) {
	...
	if err := t.saveAndBroadcastMessage(msg, asUid, msg.Pub.NoEcho, attachments, msg.Pub.Head, msg.Pub.Content); err != nil {
		globals.l.Sugar().Errorf("topic[%s]: failed to save messagge - %s", t.name, err)
		return
	}
...
func (t *Topic) handleNoteBroadcast(msg *ClientComMessage) {
	...
	if seq > 0 {
		...
		if err := globals.store.SubsUpdate(topicName, asUid, upd); err != nil {
			globals.l.Sugar().Warnf("topic[%s]: failed to update SeqRead/Recv counter: %v", t.name, err)
			return
		}
...
// User requests or updates a self-subscription to a topic. Called as a result of {sub} or {meta set=sub}.
//
...
func (t *Topic) thisUserSub(sess *Session, pkt *ClientComMessage, asUid types.Uid, asChan bool, want string,
	...
	if !existingSub || userData.deleted {
		...
		if t.cat == types.TopicCatP2P {
			...
			sub, err = globals.store.SubsGetSubs(pkt.Original, asUid, false)
			if err != nil {
			...
			if !existingSub {
				...
				sub, err = globals.store.SubsGetSubs(t.name, asUid, true)
				if err != nil {
					sess.queueOut(ErrUnknownReply(pkt, now))
					return nil, err
				}
		...
		if sub == nil || sub.DeletedAt != nil {
			...
			if err := globals.store.SubsCreate(sub); err != nil {
				sess.queueOut(ErrUnknownReply(pkt, now))
				return nil, err
			}
			...
			if err := globals.store.SubsUpdate(tname, asUid,
		...
		if len(update) > 0 {
			if err := globals.store.SubsUpdate(t.name, asUid, update); err != nil {
				sess.queueOut(ErrUnknownReply(pkt, now))
				return nil, err
			}
			pluginSubscription(&sub, plgActUpd)
		...
		if ownerChange {
			...
			if err := globals.store.SubsUpdate(t.name, t.owner,
				map[string]any{
			...
			if err := globals.store.TopOwnerChange(t.name, asUid); err != nil {
				return nil, err
			}
...
func (t *Topic) anotherUserSub(sess *Session, asUid, target types.Uid, asChan bool,
	...
	if !existingSub || userData.deleted {
		...
		sub, err := globals.store.SubsGetSubs(t.name, target, true)
		if err != nil {
			sess.queueOut(ErrUnknownReply(pkt, now))
			return nil, err
		}
		...
		if sub != nil {
			...
			if user, err := globals.store.UsersGet(target); err != nil {
				sess.queueOut(ErrUnknownReply(pkt, now))
				return nil, err
			} else if user == nil {
		...
		if modeGiven == types.ModeUnset {
			...
			if err := globals.store.SubsUpdate(t.name, target,
...
func (t *Topic) replySetDesc(sess *Session, asUid types.Uid, asChan bool,
	...
	if err == nil && len(sub) > 0 {
		...
		err = globals.store.SubsUpdate(tname, asUid, sub)
	}
	...
	if len(core) > 0 && msg.Extra != nil && len(msg.Extra.Attachments) > 0 {
		if err := globals.store.FilesLinkAttachments(t.name, types.ZeroUid, msg.Extra.Attachments); err != nil {
			globals.l.Sugar().Warnf("topic[%s] failed to link avatar attachment: %v", t.name, err)
...
// replyGetSub is a response to a get.sub request on a topic - load a list of
//
// subscriptions/subscribers, send it just to the session as a {meta} packet
func (t *Topic) replyGetSub(sess *Session, asUid types.Uid, authLevel auth.Level, asChan bool, msg *ClientComMessage) error {
	...
	case types.TopicCatMe:
		...
		if ifModified.IsZero() {
			...
			subs, err = globals.store.UsersGetTopics(asUid, msgOpts2storeOpts(req))
		} else {

			// User manages cache. Include deleted subscriptions too.
			subs, err = globals.store.UsersGetTopicsAny(asUid, msgOpts2storeOpts(req))
			...
			if err == nil && subs == nil && query != "" {
				...
				if req, opt, err = parseSearchQuery(query, sess.countryCode, rewriteLogin); err == nil {
					if len(req) > 0 || len(opt) > 0 {
						...
						allReq := globals.utils.FlattenDoubleSlice(req)
						...
						subs, err = globals.store.UsersFindSubs(asUid, req, opt, sess.authLvl != auth.LevelRoot)
						if err != nil {
	...
	case types.TopicCatP2P:
		...
		if ifModified.IsZero() {
			// No cache management. Skip deleted subscriptions.
			subs, err = globals.store.TopGetSubs(t.name, msgOpts2storeOpts(req))
		} else {
			// User manages cache. Include deleted subscriptions too.
			subs, err = globals.store.TopGetSubsAny(t.name, msgOpts2storeOpts(req))
		}
	...
	case types.TopicCatGrp:
		...
		if ifModified.IsZero() {
			// No cache management. Skip deleted subscriptions.
			subs, err = globals.store.TopGetUsers(topicName, msgOpts2storeOpts(req))

		} else {
			// User manages cache. Include deleted subscriptions too.
			subs, err = globals.store.TopGetUsersAny(topicName, msgOpts2storeOpts(req))
		}
	}
...
// replyGetData is a response to a get.data request - load a list of stored messages, send them
//
// to session as {data} response goes to a single session rather than all sessions in a topic
func (t *Topic) replyGetData(sess *Session, asUid types.Uid, asChan bool, req *MsgGetOpts, msg *ClientComMessage) error {
	...
	if userData := t.perUser[asUid]; (userData.modeGiven & userData.modeWant).IsReader() {
		...
		messages, err := globals.store.MsgGetAll(t.name, asUid, msgOpts2storeOpts(req))
		if err != nil {
			sess.queueOut(ErrUnknownReply(msg, now))
			return err
		}
	...
	if t.cat != types.TopicCatMe && t.cat != types.TopicCatGrp {
		...
		if !restrictedTagsEqual(t.tags, tags, globals.immutableTagNS) {
			...
			if len(added) > 0 || len(removed) > 0 {
				...
				if t.cat == types.TopicCatMe {
					err = globals.store.UsersUpdate(asUid, update)
				} else if t.cat == types.TopicCatGrp {
					err = globals.store.TopUpdate(t.name, update)
				}
...
func (t *Topic) replyGetCreds(sess *Session, asUid types.Uid, msg *ClientComMessage) error {
	...
	screds, err := globals.store.UsersGetAllCreds(asUid, "", false)
	if err != nil {
...
// replyGetDel is a response to a get[what=del] request: load a list of
//
// deleted message ids, send them to a session as {meta}
//
...
func (t *Topic) replyGetDel(sess *Session, asUid types.Uid, req *MsgGetOpts, msg *ClientComMessage) error {
	...
	if userData := t.perUser[asUid]; (userData.modeGiven & userData.modeWant).IsReader() {
		ranges, delID, err := globals.store.MsgGetDeleted(t.name, asUid, msgOpts2storeOpts(req))
		if err != nil {
			sess.queueOut(ErrUnknownReply(msg, now))
			return err
		}
...
func (t *Topic) replyDelMsg(sess *Session, asUid types.Uid, asChan bool, msg *ClientComMessage) error {
	...
	if len(del.DelSeq) == 0 {
		...
		if count > constants.DefaultMaxDeleteCount && len(ranges) > 1 {
	...
	if err = globals.store.MsgDeleteList(t.name, t.delID+1, forUser, ranges); err != nil {
		sess.queueOut(ErrUnknownReply(msg, now))
		return err
	}
...
func (t *Topic) replyDelTopic(sess *Session, asUid types.Uid, msg *ClientComMessage) error {
	...
	globals.l.Sugar().Errorf("replyDelTopic called by owner (SHOULD NOT HAPPEN!)")
	return nil
}
...
func (t *Topic) replyDelSub(sess *Session, asUid types.Uid, msg *ClientComMessage) error {
	...
	if err := globals.store.SubsDelete(t.name, uid); err != nil {
		if err == types.ErrNotFound {
			sess.queueOut(InfoNoActionReply(msg, now))
