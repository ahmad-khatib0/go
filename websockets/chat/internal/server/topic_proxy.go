	...
	for {
		...
		case msg := <-t.reg:
			...
			if t.isInactive() {
				...
				globals.l.Sugar().Warnf("proxy topic[%s]: route join request from proxy to master failed - %s", t.name, err)
		...
		case msg := <-t.unreg:
			if !t.handleProxyLeaveRequest(msg, killTimer) {
				...
				globals.l.Sugar().Warnf("proxy topic[%s]: failed to update proxy topic state for leave request - sid %s", t.name, sid)
				msg.sess.queueOut(ErrClusterUnreachableReply(msg, types.TimeNow()))
			}
		...
		case msg := <-t.clientMsg:
			...
			if err := globals.cluster.routeToTopicMaster(ProxyReqBroadcast, msg, t.name, msg.sess); err != nil {
				globals.l.Sugar().Warnf("topic proxy[%s]: route broadcast request from proxy to master failed - %s", t.name, err)
				msg.sess.queueOut(ErrClusterUnreachableReply(msg, types.TimeNow()))
			}
		...
		case msg := <-t.serverMsg:
			if msg.Info != nil || msg.Pres != nil {
				...
				globals.l.Sugar().Errorf("ERROR!!! topic proxy[%s]: unexpected server-side message in proxy topic %s", t.name, msg.describe())
			}

		case msg := <-t.meta:
			// Request to get/set topic metadata
			if err := globals.cluster.routeToTopicMaster(ProxyReqMeta, msg, t.name, msg.sess); err != nil {
				globals.l.Sugar().Warnf("proxy topic[%s]: route meta request from proxy to master failed - %s", t.name, err)
				msg.sess.queueOut(ErrClusterUnreachableReply(msg, types.TimeNow()))
			}
		...
		case upd := <-t.supd:
			...
			if upd.sess != nil {
				...
				if !ok {
					globals.l.Sugar().Warnf("proxy topic[%s]: sess update request from detached session - sid %s", t.name, upd.sess.sid)
					continue
				}
			...
			if err := globals.cluster.routeToTopicMaster(req, nil, t.name, tmpSess); err != nil {
				globals.l.Sugar().Warnf("proxy topic[%s]: route sess update request from proxy to master failed - %s", t.name, err)
			}
		...
		case sd := <-t.exit:
			...
			if err := globals.cluster.topicProxyGone(t.name); err != nil {
				globals.l.Sugar().Warnf("proxy topic[%s] shutdown: failed to notify master - %s", t.name, err)
			}
...
func (t *Topic) handleProxyLeaveRequest(msg *ClientComMessage, killTimer *time.Timer) bool {
	...
	if asUid.IsZero() {
		if pssd, ok := t.sessions[msg.sess]; ok {
			...
			globals.l.Sugar().Warnf("proxy topic[%s]: leave request sent for unknown session", t.name)
			return false
		}
	}
	...
	if err := globals.cluster.routeToTopicMaster(ProxyReqLeave, msg, t.name, msg.sess); err != nil {
		globals.l.Sugar().Warnf("proxy topic[%s]: route leave request from proxy to master failed - %s", t.name, err)
	}
	if len(t.sessions) == 0 {
		// No more sessions attached. Start the countdown.
		killTimer.Reset(constants.IdleProxyTopicTimeout)
	}
	return result
}
...
func (t *Topic) proxyMasterResponse(msg *ClusterResp, killTimer *time.Timer) {
	...
	keepAlive := constants.IdleProxyTopicTimeout
	...
	if msg.OrigSid == "*" {
		...
		if sess == nil {
			globals.l.Sugar().Warnf("proxy topic[%s]: session %s not found; already terminated?", t.name, msg.OrigSid)
		}
		...
		default:
			globals.l.Sugar().Errorf("proxy topic[%s] received response referencing unexpected request type %d",
				t.name, msg.OrigReqType)
		}

		if sess != nil && !sess.queueOut(msg.SrvMsg) {
			globals.l.Sugar().Errorf("proxy topic[%s]: timeout in sending response - sid %s", t.name, sess.sid)
		}
	}
}
...
func (t *Topic) proxyCtrlBroadcast(msg *ServerComMessage) {
	if msg.Ctrl.Code == http.StatusResetContent && msg.Ctrl.Text == "evicted" {
		...
		if msg.uid.IsZero() {
			globals.l.Sugar().Panicf("proxy topic[%s]: proxy received evict message with empty uid", t.name)
		}
...
func (t *Topic) updateAcsFromPresMsg(pres *MsgServerPres) {
	...
	if uid.IsZero() {
		if t.cat != types.TopicCatMe {
			globals.l.Sugar().Warnf("proxy topic[%s]: received acs change for invalid user id '%s'", t.name, pres.Src)
		}
		return
	}
	...
	if err := pud.modeWant.ApplyMutation(dacs.Want); err != nil {
		globals.l.Sugar().Warnf("proxy topic[%s]: could not process acs change - want: %s", t.name, err)
		return
	}
	if err := pud.modeGiven.ApplyMutation(dacs.Given); err != nil {
		globals.l.Sugar().Warnf("proxy topic[%s]: could not process acs change - given: %s", t.name, err)
		return
	}
