...
func normalizeTags(src []string) types.StringSlice {
	...
	for _, curr := range src {
		...
		if len(ucurr) < constants.MinTagLength || len(ucurr) > constants.MaxTagLength || curr == prev {
			continue
		}
...
func selectAccessMode(authLvl at.Level, anonMode, authMode, rootMode types.AccessMode) types.AccessMode {
	switch authLvl {
	case at.LevelNone:
...
func rewriteTag(orig, countryCode string, withLogin bool) string {
	...
	if withLogin {
		auths := globals.store.AuthGetAuthNames()
		for _, name := range auths {
			auth := globals.store.GetAuthHandler(name)
	...
	globals.l.Sugar().Warnf("invalid generic tag '%s'", orig)
	return ""
}
