...
func topicInit(t *Topic, join *ClientComMessage, h *Hub) {
	...
	if err != nil {
		...
		globals.l.Sugar().Errorf("init_topic: failed to load or create topic:", join.RcptTo, err)
	...
	globals.stats.IntStatsInc("TotalTopics", 1)
...
func initTopicMe(t *Topic, sreg *ClientComMessage) error {
	...
	user, err := globals.store.UsersGet(types.ParseUserId(t.name))
	if err != nil {
		// Log out the session
...
func initTopicP2P(t *Topic, sreg *ClientComMessage) error {
	...
	stopic, err := globals.store.TopGet(t.name)
	if err != nil {
		return err
	}
	...
	if stopic != nil {
		...
		if subs, err = globals.store.TopGetUsers(t.name, nil); err != nil {
			return err
		}

		// Case 3, fail
		if len(subs) == 0 {
			globals.l.Sugar().Errorf("hub: missing both subscriptions for '" + t.name + "' (SHOULD NEVER HAPPEN!)")
	...
	if stopic != nil && len(subs) == 2 {
		...
		var u1, u2 int
		users, err := globals.store.UsersGetAll(userID1, userID2)
		if err != nil {
			return err
		}
		...
		if sub2 == nil {
			...
			if pktsub.Set != nil && pktsub.Set.Desc != nil && pktsub.Set.Desc.DefaultAcs != nil {
				...
				if err := sub2.ModeGiven.UnmarshalText([]byte(pktsub.Set.Desc.DefaultAcs.Auth)); err != nil {
					globals.l.Sugar().Errorf("hub: invalid access mode", t.xoriginal, pktsub.Set.Desc.DefaultAcs.Auth)
				}
			...
			if pktsub.Set != nil {
				if pktsub.Set.Sub != nil {
					...
					if uid != userID1 {
						...
						globals.l.Sugar().Errorf("hub: setting mode for another user is not supported '" + t.name + "'")
					} else {
						// user1 is setting non-default modeWant
						if err := userData.modeWant.UnmarshalText([]byte(pktsub.Set.Sub.Mode)); err != nil {
							globals.l.Sugar().Errorf("hub: invalid access mode", t.xoriginal, pktsub.Set.Sub.Mode)
						}
						// Ensure sanity
		...
		// Create everything
		if stopic == nil {
			if err = globals.store.TopCreateP2P(sub1, sub2); err != nil {
				return err
			}
			...
			if err = globals.store.SubsCreate(subToMake); err != nil {
				return err
			}
		}
...
func initTopicNewGrp(t *Topic, sreg *ClientComMessage, isChan bool) error {
	...
	if pktsub.Set != nil {
		...
		if pktsub.Set.Desc != nil {
			if pktsub.Set.Desc.Trusted != nil && authLevel != auth.LevelRoot {
				globals.l.Sugar().Errorf("hub: attempt to assign Trusted by non-ROOT", t.name)
			...
			if pktsub.Set.Desc.DefaultAcs != nil {
				if authMode, anonMode, err := parseTopicAccess(pktsub.Set.Desc.DefaultAcs,
					...
					globals.l.Sugar().Errorf("hub: invalid access mode for topic '" + t.name + "': '" + err.Error() + "'")
				} else if authMode.IsOwner() || anonMode.IsOwner() {
					globals.l.Sugar().Errorf("hub: OWNER default access in topic", t.name)
	...
	err := globals.store.TopCreate(stopic, t.owner, t.perUser[t.owner].private)
	if err != nil {
		return err
	}
	...
	if sreg.Extra != nil && len(sreg.Extra.Attachments) > 0 {
		if err := globals.store.FilesLinkAttachments(t.name, types.ZeroUid, sreg.Extra.Attachments); err != nil {
...
func (t *Topic) loadSubscribers() error {
	subs, err := globals.store.TopGetSubs(t.name, nil)
	if err != nil {
		return err
	}
