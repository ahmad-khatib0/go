	...
	go func() {
		...
		if server.TLSConfig != nil {
			...
			if globals.tlsRedirectHTTP != "" {
				...
				if isUnixAddr(globals.tlsRedirectHTTP) || isUnixAddr(addr) {
					...
					globals.l.Sugar().Infof("Redirecting connections from HTTP at [%s] to HTTPS at [%s]",
					...
					go func() {
						if err := http.ListenAndServe(globals.tlsRedirectHTTP, tlsRedirect(addr)); err != nil && err != http.ErrServerClosed {
							globals.l.Sugar().Infof("HTTP redirect failed:", err)
						}
					}()
				}
			}
			...
			if err == nil {
				globals.l.Sugar().Infof("Listening for client HTTPS connections on [%s]", addr)
			...
			globals.l.Sugar().Infof("Listening for client HTTP connections on [%s]", addr)
			var lis net.Listener
			lis, err = netListener(addr)
			if err == nil {
		...
		if err != nil {
			if globals.shuttingDown {
				globals.l.Sugar().Infof("HTTP server: stopped")
			} else {
				globals.l.Sugar().Errorf("HTTP server: failed", err)
			}
		}
		httpdone <- true
	}()
	...
	for {
		...
		case <-stop:
			...
			if err := server.Shutdown(ctx); err != nil {
				...
				globals.l.Sugar().Errorf("HTTP server failed to terminate gracefully", err)
			}
...
func signalHandler() <-chan bool {
	...
	go func() {
		...
		sig := <-signchan
		globals.l.Sugar().Infof("Signal received: '%s', shutting down", sig)
		stop <- true
	}()
...
func authHttpRequest(req *http.Request) (types.Uid, []byte, error) {
	...
	if authMethod, secret := getHttpAuth(req); authMethod != "" {
		...
		if err != nil {
			globals.l.Sugar().Infof("media: invalid auth secret", authMethod, "'"+secret+"'")
			return uid, nil, types.ErrMalformed
		}

		if authhdl := globals.store.AuthGetLogicalAuthHandler(authMethod); authhdl != nil {
			...
			globals.l.Sugar().Infof("media: unknown auth method", authMethod)
		}
	} else {
