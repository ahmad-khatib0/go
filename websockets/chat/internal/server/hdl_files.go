...
func largeFileServe(wrt http.ResponseWriter, req *http.Request) {
	...
	globals.stats.IntStatsInc("FileDownloadsTotal", 1)
	...
	writeHttpResponse := func(msg *ServerComMessage, err error) {
		...
		enc.Encode(msg)
		if err != nil {
			globals.l.Sugar().Warnf("media serve:", req.URL.String(), err)
		}
	}
	...
	if req.Method == http.MethodOptions {
		...
		globals.l.Sugar().Infof("media serve: preflight completed")
		return
	}
	...
	if statusCode != 0 {
		...
		globals.l.Sugar().Infof("media serve: completed with status", statusCode, "uid=", uid)
		return
	}

	if req.Method == http.MethodHead {
		wrt.WriteHeader(http.StatusOK)
		globals.l.Sugar().Infof("media serve: completed", req.Method, "uid=", uid)
		return
	}
	...
	globals.l.Sugar().Infof("media serve: OK, uid=", uid)
}
...
func largeFileReceive(wrt http.ResponseWriter, req *http.Request) {
	...
	writeHttpResponse := func(msg *ServerComMessage, err error) {
		...
		if err != nil {
			globals.l.Sugar().Infof("media upload:", msg.Ctrl.Code, msg.Ctrl.Text, "/", err)
		}
	}
	...
	if req.Method == http.MethodOptions {
		...
		wrt.WriteHeader(statusCode)
		globals.l.Sugar().Infof("media upload: preflight completed")
		return
	}
	...
	headers, statusCode, err := mh.Headers(req, false)
	if err != nil {
		globals.l.Sugar().Infof("media upload: headers check failed", err)
		writeHttpResponse(decodeStoreError(err, "", now, nil), err)
		return
	}
	...
	if statusCode != 0 {
		...
		globals.l.Sugar().Infof("media upload: completed with status", statusCode)
		return
	}

	if req.Method == http.MethodHead || req.Method == http.MethodOptions {
		wrt.WriteHeader(http.StatusOK)
		globals.l.Sugar().Infof("media upload: completed", req.Method)
		return
	}

	file, header, err := req.FormFile("file")
	if err != nil {
		globals.l.Sugar().Infof("media upload: invalid multipart form", err)
	...
	globals.l.Sugar().Infof("media upload: ok", fdef.Id, fdef.Location)
}
