...
func (t *Topic) loadContacts(uid types.Uid) error {
	subs, err := globals.store.UsersGetSubs(uid)
	if err != nil {
		return err
	}
...
func (t *Topic) infoCallSubsOffline(
	from string,
	target types.Uid,
	event string,
	seq int,
	...
	if target.IsZero() {
		globals.l.Sugar().Errorf("callSubs could not find target: topic %s - from %s", t.name, from)
		return
	}
...
// Publish to topic subscribers's sessions currently offline in the topic, on their 'me' Group and P2P.
//
...
func (t *Topic) presSubsOffline(
	what string,
	params *presParams,
	filterSource *presFilters,
	filterTarget *presFilters,
	skipSid string,
	offlineOnly bool,
) {
...
//
// If both 'read' and 'recv' != 0 then 'read' takes precedence over 'recv'. Cases U
...
// Let other sessions of a given user know that messages are now deleted,, Cases V.1, V.2
func (t *Topic) presPubMessageDelete(uid types.Uid, mode types.AccessMode, delID int, list []MsgDelRange, skip string) {
	if len(list) == 0 && delID <= 0 {
		globals.l.Sugar().Warnf("Case V.1, V.2: topic[%s] invalid request - missing payload", t.name)
		return
	}
