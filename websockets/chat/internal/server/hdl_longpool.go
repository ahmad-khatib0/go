...
func (sess *Session) sendMessageLp(wrt http.ResponseWriter, msg any) bool {
	if len(sess.send) > sendQueueLimit {
		...
		globals.l.Sugar().Errorf("longPoll: outbound queue limit exceeded", sess.sid)
		return false
	}

	globals.stats.IntStatsInc("OutgoingMessagesLongpollTotal", 1)
	if err := lpWrite(wrt, msg); err != nil {
		globals.l.Sugar().Errorf("longPoll: writeOnce failed", sess.sid, err)
		return false
	}
	...
	for {
		...
		case <-time.After(pingPeriod):
			...
			if _, err := wrt.Write([]byte{}); err != nil {
				globals.l.Sugar().Errorf("longPoll: writeOnce: timout", sess.sid, err)
			}
			return
...
func (sess *Session) readOnce(wrt http.ResponseWriter, req *http.Request) (int, error) {
	...
	if err == nil {
		...
		sess.lock.Lock()
		globals.stats.IntStatsInc("IncomingMessagesLongpollTotal", 1)
...
func serveLongPoll(wrt http.ResponseWriter, req *http.Request) {
	...
	if sid == "" {
		...
		globals.l.Sugar().Infof("longPoll: session started", sess.sid, sess.remoteAddr, count)
	...
	if sess == nil {
		globals.l.Sugar().Warnf("longPoll: invalid or expired session id", sid)
		wrt.WriteHeader(http.StatusForbidden)
	...
	if addr := getRemoteAddr(req); sess.remoteAddr != addr {
		...
		globals.l.Sugar().Warnf("longPoll: remote address changed", sid, addr)
	}

	if req.ContentLength != 0 {
		// Read payload and send it for processing.
		if code, err := sess.readOnce(wrt, req); err != nil {
			globals.l.Sugar().Warnf("longPoll: readOnce failed", sess.sid, err)
