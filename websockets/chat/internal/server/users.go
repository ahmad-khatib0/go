...
import (
	...
	push "github.com/ahmad-khatib0/go/websockets/chat/internal/push/types"
...
func InitUsersGarbageCollection(period time.Duration, blockSize, minAccountAgeHours int) chan<- bool {
	...
	go func() {
		...
		for {
			select {
			case <-gt:
				if uids, err := globals.store.UsersGetUnvalidated(time.Now().Add(-staleAge), blockSize); err == nil {

					if len(uids) > 0 {
						globals.l.Sugar().Infof("stale account GC will delete these uids: ", uids)
						for _, id := range uids {
							if err := globals.store.UsersDelete(id, true); err != nil {
								globals.l.Sugar().Warnf("stale account GC failed to delete %s: %+v", id.UserId(), err)
							}
						}
					}

				} else {
					globals.l.Sugar().Infof("stale account GC error %w", err)
				}
...
func replyCreateUser(s *Session, msg *ClientComMessage, rec *auth.Rec) {
	...
	if msg.Acc.Login && (!s.uid.IsZero() || rec != nil) {
		...
		globals.l.Sugar().Warnf("create user: login requested while authenticated, sid=", s.sid)
		return
	}

	// Find authenticator for the requested scheme.
	authhdl := globals.store.AuthGetLogicalAuthHandler(msg.Acc.Scheme)
	if authhdl == nil {
		// New accounts must have an authentication scheme
		s.queueOut(ErrMalformed(msg.Id, "", msg.Timestamp))
		globals.l.Sugar().Warnf("create user: unknown auth handler, sid=", s.sid)
		return
	}

	// Check if login is unique.
	if ok, err := authhdl.IsUnique(msg.Acc.Secret, s.remoteAddr); !ok {
		globals.l.Sugar().Warnf("create user: auth secret is not unique", err, "sid=", s.sid)
	...
	if msg.Acc.State != "" {
		if auth.Level(msg.AuthLvl) != auth.LevelRoot {
			globals.l.Sugar().Warnf("create user: attempt to set account state by non-root, sid=", s.sid)
		...
		if err != nil || state == types.StateUndefined || state == types.StateDeleted {
			globals.l.Sugar().Warnf("create user: invalid account state", err, "sid=", s.sid)
			s.queueOut(ErrMalformed(msg.Id, "", msg.Timestamp))
			return
		}
		user.State = state
	...
	if tags := normalizeTags(msg.Acc.Tags); tags != nil {
		if !restrictedTagsEqual(tags, nil, globals.immutableTagNS) {
			globals.l.Sugar().Warnf("create user: attempt to directly assign restricted tags, sid=", s.sid)
			msg := ErrPermissionDenied(msg.Id, "", msg.Timestamp)
	...
	for i := range creds {
		cr := &creds[i]
		vld := globals.store.GetValidator(cr.Method)
		if _, err := vld.PreCheck(cr.Value, cr.Params); err != nil {
			globals.l.Sugar().Warnf("create user: failed credential pre-check", cr, err, "sid=", s.sid)
			s.queueOut(decodeStoreError(err, msg.Id, msg.Timestamp,
	...
	if _, err := globals.store.UsersCreate(&user, private); err != nil {
		globals.l.Sugar().Warnf("create user: failed to create user", err, "sid=", s.sid)
	...
	if err != nil {
		globals.l.Sugar().Warnf("create user: add auth record failed", err, "sid=", s.sid)
		// Attempt to delete incomplete user record
		if err = globals.store.UsersDelete(user.Uid(), true); err != nil {
			globals.l.Sugar().Warnf("create user: failed to delete incomplete user record", err, "sid=", s.sid)
		}
	...
	if len(creds) < len(globals.authValidators[rec.AuthLevel]) {
		globals.l.Sugar().Warnf("create user: missing credentials; have:", creds, "want:",
	...
	tmpToken, _, _ := globals.store.AuthGetLogicalAuthHandler("token").GenSecret(&auth.Rec{
		Uid:       user.Uid(),
	...
	if err != nil {
		globals.l.Sugar().Warnf("create user: failed to save or validate credential", err, "sid=", s.sid)
		s.queueOut(decodeStoreError(err, msg.Id, msg.Timestamp, nil))
	...
	if msg.Extra != nil && len(msg.Extra.Attachments) > 0 {
		if err := globals.store.FilesLinkAttachments(user.Uid().UserId(), types.ZeroUid, msg.Extra.Attachments); err != nil {
			globals.l.Sugar().Warnf("create user: failed to link avatar attachment", err, "sid=", s.sid)
...
func replyUpdateUser(s *Session, msg *ClientComMessage, rec *auth.Rec) {
	if s.uid.IsZero() && rec == nil {
		...
		globals.l.Sugar().Warnf("replyUpdateUser: not a new account and not authenticated", s.sid)
		...
		globals.l.Sugar().Warnf("replyUpdateUser: got both authenticated session and token", s.sid)
		s.queueOut(ErrMalformed(msg.Id, "", msg.Timestamp))
		return
	}
	...
	if msg.Acc.User != "" && msg.Acc.User != userId {
		if s.authLvl != auth.LevelRoot {
			globals.l.Sugar().Warnf("replyUpdateUser: attempt to change another's account by non-root", s.sid)
			s.queueOut(ErrPermissionDenied(msg.Id, "", msg.Timestamp))
			return
		}
	...
	if uid.IsZero() {
		...
		s.queueOut(ErrMalformed(msg.Id, "", msg.Timestamp))
		globals.l.Sugar().Warnf("replyUpdateUser: user id is invalid or missing", s.sid)
		return
	}
	...
	if msg.Acc.State != "" && s.authLvl != auth.LevelRoot {
		s.queueOut(ErrPermissionDenied(msg.Id, "", msg.Timestamp))
		globals.l.Sugar().Warnf("replyUpdateUser: attempt to change account state by non-root", s.sid)
		return
	}

	user, err := globals.store.UsersGet(uid)
	if user == nil && err == nil {
		err = types.ErrNotFound
	}
	if err != nil {
		globals.l.Sugar().Warnf("replyUpdateUser: failed to fetch user from DB", err, s.sid)
		s.queueOut(decodeStoreError(err, msg.Id, msg.Timestamp, nil))
		return
	}
	...
	if msg.Acc.Scheme != "" {
		...
		if authLvl == auth.LevelNone {
			...
			s.queueOut(ErrMalformed(msg.Id, "", msg.Timestamp))
			globals.l.Sugar().Warnf("replyUpdateUser: auth level is missing", s.sid)
			return
		}
		...
		if err == nil {
			if allCreds, err := globals.store.UsersGetAllCreds(uid, "", true); err != nil {
	...
	if err != nil {
		globals.l.Sugar().Warnf("replyUpdateUser: failed to update user", err, s.sid)
		s.queueOut(decodeStoreError(err, msg.Id, msg.Timestamp, nil))
		return
	}
...
func updateUserAuth(msg *ClientComMessage, user *types.User, rec *auth.Rec, remoteAddr string) error {
	...
	if authhdl != nil {
		...
		if _, err = globals.store.UsersUpdateTags(user.Uid(), nil, nil, rec.Tags); err != nil {
			globals.l.Sugar().Warnf("updateUserAuth tags update failed:", err)
		}
		return nil
	}
...
func addCreds(uid types.Uid, creds []MsgCredClient, extraTags []string,
	...
	if len(extraTags) > 0 {
		if utags, err := globals.store.UsersUpdateTags(uid, extraTags, nil, nil); err == nil {
			extraTags = utags
		} else {
			globals.l.Sugar().Warnf("add cred tags update failed:", err)
		}
	} else {
		extraTags = nil
	}
...
func validatedCreds(uid types.Uid, authLvl auth.Level, creds []MsgCredClient,
	...
	allCreds, err := globals.store.UsersGetAllCreds(uid, "", true)
	if err != nil {
		return nil, nil, err
	}
	...
	for i := range creds {
		...
		vld := globals.store.GetValidator(cr.Method) // No need to check for nil, unknown methods are removed earlier.
	...
	if len(tagsToAdd) > 0 {
		// Save update to tags
		if utags, err := globals.store.UsersUpdateTags(uid, tagsToAdd, nil, nil); err == nil {
			tags = utags
		} else {
			globals.l.Sugar().Warnf("validated creds tags update failed:", err)
			tags = nil
		}
	} else {
		tags = nil
...
func deleteCred(uid types.Uid, authLvl auth.Level, cred *MsgCredClient) ([]string, error) {
	vld := globals.store.GetValidator(cred.Method)
	...
	if isRequired {
		...
		allCreds, err := globals.store.UsersGetAllCreds(uid, cred.Method, false)
		if err != nil {
			return nil, err
		}
	...
	if globals.validators[cred.Method].AddToTags {
		...
		if utags, err := globals.store.UsersUpdateTags(uid, nil, []string{cred.Method + ":" + cred.Value}, nil); err == nil {
			tags = utags
		} else {
			globals.l.Sugar().Warnf("delete cred: failed to update tags:", err)
			tags = nil
		}
	} else {
		tags = nil
...
func changeUserState(s *Session, uid types.Uid, user *types.User, msg *ClientComMessage) (bool, error) {
	...
	if err != nil || state == types.StateUndefined {
		globals.l.Sugar().Warnf("replyUpdateUser: invalid account state", s.sid)
	...
	err = globals.store.UsersUpdateState(uid, state)
	if err != nil {
		return false, err
	}
...
func replyDelUser(s *Session, msg *ClientComMessage) {
	...
	if msg.Del.User == "" || msg.Del.User == s.uid.UserId() {
		...
		if globals.permanentAccounts && s.authLvl != auth.LevelRoot {
			globals.l.Sugar().Warnf("replyDelUser: account deletion disabled", s.sid)
		...
		if uid.IsZero() {
			globals.l.Sugar().Warnf("replyDelUser: invalid user ID", msg.Del.User, s.sid)
			s.queueOut(ErrMalformed(msg.Id, "", msg.Timestamp))
			return
		}
	} else {
		globals.l.Sugar().Warnf("replyDelUser: illegal attempt to delete another user", msg.Del.User, s.sid)
		s.queueOut(ErrPermissionDenied(msg.Id, "", msg.Timestamp))
		return
	}

	// Disable all authenticators
	authnames := globals.store.AuthGetAuthNames()
	for _, name := range authnames {
		hdl := globals.store.AuthGetLogicalAuthHandler(name)
		...
		if err := hdl.DelRecords(uid); err != nil {
			...
			globals.l.Sugar().Warnf("replyDelUser: failed to delete auth record", uid.UserId(), name, err, s.sid)
	...
	if uoi, err := globals.store.UsersGetSubs(uid); err == nil {
		presUsersOfInterestOffline(uid, uoi, "gone")
	} else {
		globals.l.Sugar().Warnf("replyDelUser: failed to send notifications to users", err, s.sid)
	}

	// Notify subscribers of the group topics where the user was the owner that the topics were deleted.
	if ownTopics, err := globals.store.UsersGetOwnTopics(uid); err == nil {
		for _, topicName := range ownTopics {
			if subs, err := globals.store.TopGetSubs(topicName, nil); err == nil {
				presSubsOfflineOffline(topicName, types.TopicCatGrp, subs, "gone", &presParams{}, s.sid)
			} else {
				globals.l.Sugar().Warnf("replyDelUser: failed to notify topic subscribers", err, topicName, s.sid)
			}
		}
	} else {
		globals.l.Sugar().Warnf("replyDelUser: failed to send notifications to owned topics", err, s.sid)
	}

	// TODO: suspend all P2P topics with the user.

	// Delete user's records from the database.
	if err := globals.store.UsersDelete(uid, msg.Del.Hard); err != nil {
		globals.l.Sugar().Warnf("replyDelUser: failed to delete user", err, s.sid)
		s.queueOut(decodeStoreError(err, msg.Id, msg.Timestamp, nil))
		return
	}
...
func userChannelsSubUnsub(uid types.Uid, deviceID string, sub bool) {
	globals.push.ChannelSub(&push.ChannelReq{
		Uid:      uid,
		DeviceID: deviceID,
		Unsub:    !sub,
	})
...
func usersRegisterTopic(t *Topic, add bool) {
	...
	if len(local.UserIdList) > 0 {
		...
		default:
			globals.l.Sugar().Errorf("User cache: globals.usersUpdate queue full: ", len(globals.usersUpdate))
		}
	}
}
...
func userUpdater() {
	...
	unreadUpdater := func(uids []types.Uid, vals []int, inc bool) map[types.Uid]int {
		...
		for i, uid := range uids {
			...
			if !ok {
				globals.l.Sugar().Errorf("ERROR: attempt to update unread count for user who has not been loaded", uid)
		...
		if len(dbPending) > 0 {
			go func() {
				dbUnread, err := globals.store.UsersGetUnreadCount(dbPending...)
				if err != nil {
					globals.l.Sugar().Warnf("users: failed to load unread count: ", err)
				}
		...
		case io := <-ioDone:
			...
			for uid, count := range io.counts {
				...
				if ok {
					...
					globals.l.Sugar().Warnf("ERROR: io didn't have an update buffer, uid", uid)
				}

				if uce, ok := usersCache[uid]; ok {
					if uce.unread >= 0 {
						globals.l.Sugar().Warnf("users: unread count double initialization, uid", uid)
					}
					uce.unread = count
					usersCache[uid] = uce
				} else {
					globals.l.Sugar().Warnf("users: missing users cache entry after IO completion, uid", uid)
				}
			...
			if io.err != nil {
				globals.l.Sugar().Errorf("users: failed to read unread count:", io.err)
				continue
			}
		...
		case upd := <-globals.usersUpdate:
			...
			if upd.PushRcpt != nil {
				...
				if len(pendingUsers) == 0 {
					...
					globals.push.Push(upd.PushRcpt)
				} else {
			...
			if len(upd.UserIdList) > 0 {
				for _, uid := range upd.UserIdList {
					...
					if upd.Inc {
						...
						// BUG!
						globals.l.Sugar().Errorf("ERROR: request to unregister user which has not been registered", uid)
					}
				}
				continue
			}
...
Exit:
	globals.l.Sugar().Infof("users: shutdown")
}
	...
	go func() {
		...
		for {
			...
			case <-gcTicker:
				if uids, err := globals.store.UsersGetUnvalidated(time.Now().Add(-staleAge), blockSize); err == nil {
					if len(uids) > 0 {
						globals.l.Sugar().Infof("Stale account GC will delete uids:", uids)
						for _, uid := range uids {
							if err = globals.store.UsersDelete(uid, true); err != nil {
								globals.l.Sugar().Warnf("Stale account GC failed to delete %s: %+v", uid.UserId(), err)
							}
						}
					}
				} else {
					globals.l.Sugar().Warnf("Stale account GC error:", err)
				}
			case <-stop:
				return
			}
