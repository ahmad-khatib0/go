...
func (s *Session) queueOutBatch(msgs []*ServerComMessage) bool {
	...
	if s.supportsMessageBatching() {
		...
		default:
			...
			globals.l.Sugar().Errorf("s.queueOut: session's send queue2 full", s.sid)
			return false
		}
		if s.isMultiplex() {
...
func (s *Session) queueOutBytes(data []byte) bool {
	...
	case s.send <- data:
	default:
		globals.l.Sugar().Errorf("s.queueOutBytes: session's send queue full", s.sid)
		return false
	}
	if s.isMultiplex() {
		s.scheduleClusterWriteLoop()
...
func (s *Session) dispatchRaw(raw []byte) {
	...
	if atomic.LoadInt32(&s.terminating) > 0 {
		...
		globals.l.Sugar().Warnf("s.dispatch: message received on a terminating session", s.sid)
	...
	globals.l.Sugar().Infof("in: '%s%s' sid='%s' uid='%s'", toLog, truncated, s.sid, s.uid)

	if err := json.Unmarshal(raw, &msg); err != nil {
		// Malformed message
		globals.l.Sugar().Warnf("s.dispatch", err, s.sid)
...
func (s *Session) dispatch(msg *ClientComMessage) {
	...
	if msg.Extra == nil || msg.Extra.AsUser == "" {
		...
		globals.l.Sugar().Warnf("s.dispatch: non-root asigned msg.from", s.sid)
		return
	} else if fromUid := types.ParseUserId(msg.Extra.AsUser); fromUid.IsZero() {
		// Invalid msg.Extra.AsUser.
		s.queueOut(ErrMalformed("", "", now))
		globals.l.Sugar().Warnf("s.dispatch: malformed msg.from: ", msg.Extra.AsUser, s.sid)
		return
	} else {
	...
	checkVers := func(handler func(*ClientComMessage)) func(*ClientComMessage) {
		return func(m *ClientComMessage) {
			if s.ver == 0 {
				globals.l.Sugar().Warnf("s.dispatch: {hi} is missing", s.sid)
	...
	checkUser := func(handler func(*ClientComMessage)) func(*ClientComMessage) {
		return func(m *ClientComMessage) {
			if msg.AsUser == "" {
				globals.l.Sugar().Warnf("s.dispatch: authentication required", s.sid)
	...
	default:
		...
		globals.l.Sugar().Warnf("s.dispatch: unknown message", s.sid)
		return
	}
...
func (s *Session) subscribe(msg *ClientComMessage) {
	...
	if sub := s.getSub(msg.RcptTo); sub != nil {
		...
		default:
			...
			s.inflightReqs.Done()
			globals.l.Sugar().Errorf("s.subscribe: hub.join queue full, topic ", msg.RcptTo, s.sid)
		}
...
func (s *Session) leave(msg *ClientComMessage) {
	...
	if !msg.Leave.Unsub {
		...
		globals.l.Sugar().Warnf("s.leave:", "must attach first", s.sid)
...
func (s *Session) publish(msg *ClientComMessage) {
	...
	if sub := s.getSub(msg.RcptTo); sub != nil {
		...
		default:
			// Reply with a 503 to the user.
			s.queueOut(ErrServiceUnavailableReply(msg, msg.Timestamp))
			globals.l.Sugar().Errorf("s.publish: sub.broadcast channel full, topic ", msg.RcptTo, s.sid)
		}
		...
		default:
			// Reply with a 503 to the user.
			s.queueOut(ErrServiceUnavailableReply(msg, msg.Timestamp))
			globals.l.Sugar().Errorf("s.publish: hub.route channel full", s.sid)
		}

	} else {
		// Publish request received without attaching to topic first.
		s.queueOut(ErrAttachFirst(msg, msg.Timestamp))
		globals.l.Sugar().Warnf("s.publish[%s]: must attach first %s", msg.RcptTo, s.sid)
	}
}
...
func (s *Session) hello(msg *ClientComMessage) {
	...
	if s.ver == 0 {
		...
		if s.ver == 0 {
			globals.l.Sugar().Warnf("s.hello:", "failed to parse version", s.sid)
		...
		if versionCompare(s.ver, minSupportedVersionValue) < 0 {
			...
			globals.l.Sugar().Warnf("s.hello:", "unsupported version", s.sid)
			return
		}

		params = map[string]any{
			"ver":                globals.currentVersion,
			"build":              globals.store.DBGetAdapterName() + ":" + globals.buildstamp,
			"maxMessageSize":     globals.maxMessageSize,
			"maxSubscriberCount": globals.maxSubscriberCount,
			"minTagLength":       constants.MinTagLength,
			"maxTagLength":       constants.MaxTagLength,
		...
		if !s.uid.IsZero() {
			...
			if msg.Hi.DeviceID == types.NullValue {
				deviceIDUpdate = true
				err = globals.store.DevDelete(s.uid, s.deviceID)

			} else if msg.Hi.DeviceID != "" && s.deviceID != msg.Hi.DeviceID {
				deviceIDUpdate = true
				err = globals.store.DevUpdate(s.uid, s.deviceID, &types.DeviceDef{
			...
			if err != nil {
				globals.l.Sugar().Warnf("s.hello:", "device ID", err, s.sid)
		...
		globals.l.Sugar().Warnf("s.hello:", "version cannot be changed", s.sid)
		return
	}
	...
	if s.countryCode == "" {
		if len(s.lang) > 2 {
			...
			globals.l.Sugar().Warnf("s.hello:", "could not parse locale ", s.lang)
		}
...
func (s *Session) acc(msg *ClientComMessage) {
	...
	if !newAcc && msg.Acc.TmpScheme != "" {
		...
		if !s.uid.IsZero() {
			...
			globals.l.Sugar().Warnf("s.acc: got temp auth while already authenticated", s.sid)
			return
		}

		authHdl := globals.store.AuthGetLogicalAuthHandler(msg.Acc.TmpScheme)
		if authHdl == nil {
			globals.l.Sugar().Warnf("s.acc: unknown authentication scheme", msg.Acc.TmpScheme, s.sid)
		...
		if err != nil {
			...
			globals.l.Sugar().Warnf("s.acc: invalid temp auth", err, s.sid)
			return
		}
	}
...
func (s *Session) login(msg *ClientComMessage) {
	...
	handler := globals.store.AuthGetLogicalAuthHandler(msg.Login.Scheme)
	if handler == nil {
		globals.l.Sugar().Warnf("s.login: unknown authentication scheme", msg.Login.Scheme, s.sid)
		s.queueOut(ErrAuthUnknownScheme(msg.Id, "", msg.Timestamp))
		return
	}
	...
	if err != nil {
		...
		if resp.Ctrl.Code >= 500 {
			// Log internal errors
			globals.l.Sugar().Warnf("s.login: internal", err, s.sid)
		}
	...
	if err != nil {
		globals.l.Sugar().Warnf("s.login: user state check failed", rec.Uid, err, s.sid)
		s.queueOut(decodeStoreError(err, msg.Id, msg.Timestamp, nil))
		return
	}
	...
	if err != nil {
		globals.l.Sugar().Warnf("s.login: failed to validate credentials:", err, s.sid)
		s.queueOut(decodeStoreError(err, msg.Id, msg.Timestamp, nil))
...
func (s *Session) authSecretReset(params []byte) error {
	...
	auther := globals.store.AuthGetLogicalAuthHandler(authScheme)
	if auther == nil {
		return types.ErrUnsupported
	}

	validator := globals.store.GetValidator(credMethod)
	if validator == nil {
		return types.ErrUnsupported
	}

	uid, err := globals.store.UsersGetByCred(credMethod, credValue)
	if err != nil {
		return err
	}
	...
	tempAuth := globals.store.AuthGetLogicalAuthHandler(tempScheme)
	if tempAuth == nil || !tempAuth.IsInitialized() {
		globals.l.Sugar().Errorf("s.authSecretReset: validator with missing temp auth", credMethod, tempScheme, s.sid)
...
func (s *Session) onLogin(msgID string, timestamp time.Time, rec *auth.Rec, missing []string) *ServerComMessage {
	...
	if len(missing) > 0 {
		...
		if s.deviceID != "" {
			...
			if err := globals.store.DevUpdate(rec.Uid, "", &types.DeviceDef{
				DeviceId: s.deviceID,
				Platform: s.platf,
				LastSeen: timestamp,
				Lang:     s.lang,
			}); err != nil {
				globals.l.Sugar().Warnf("failed to update device record", err)
			}
		}
	}
	...
	params["token"], params["expires"], _ = globals.store.AuthGetLogicalAuthHandler("token").GenSecret(rec)
...
func (s *Session) get(msg *ClientComMessage) {
	...
	if msg.MetaWhat == 0 {
		...
		globals.l.Sugar().Warnf("s.get: invalid Get message action", msg.Get.What)
		...
		default:
			// Reply with a 503 to the user.
			s.queueOut(ErrServiceUnavailableReply(msg, msg.Timestamp))
			globals.l.Sugar().Errorf("s.get: sub.meta channel full, topic ", msg.RcptTo, s.sid)
		}
		...
		default:
			// Reply with a 503 to the user.
			s.queueOut(ErrServiceUnavailableReply(msg, msg.Timestamp))
			globals.l.Sugar().Errorf("s.get: hub.meta channel full", s.sid)
		}

	} else {
		globals.l.Sugar().Warnf("s.get: subscribe first to get=", msg.Get.What)
		s.queueOut(ErrPermissionDeniedReply(msg, msg.Timestamp))
	}
}
...
func (s *Session) set(msg *ClientComMessage) {
	...
	if msg.MetaWhat == 0 {
		s.queueOut(ErrMalformedReply(msg, msg.Timestamp))
		globals.l.Sugar().Warnf("s.set: nil Set action")
		...
		default:
			// Reply with a 503 to the user.
			s.queueOut(ErrServiceUnavailableReply(msg, msg.Timestamp))
			globals.l.Sugar().Errorf("s.set: sub.meta channel full, topic ", msg.RcptTo, s.sid)
		}

	} else if msg.MetaWhat&(constMsgMetaTags|constMsgMetaCred) != 0 {
		globals.l.Sugar().Warnf("s.set: can Set tags/creds for subscribed topics only", msg.MetaWhat)
		s.queueOut(ErrPermissionDeniedReply(msg, msg.Timestamp))
		...
		default:
			// Reply with a 503 to the user.
			s.queueOut(ErrServiceUnavailableReply(msg, msg.Timestamp))
			globals.l.Sugar().Errorf("s.set: hub.meta channel full", s.sid)
		}
	}
}
...
func (s *Session) del(msg *ClientComMessage) {
	...
	if msg.MetaWhat == 0 {
		s.queueOut(ErrMalformedReply(msg, msg.Timestamp))
		globals.l.Sugar().Warnf("s.del: invalid Del action", msg.Del.What, s.sid)
		return
	}
	...
	if sub := s.getSub(msg.RcptTo); sub != nil && msg.MetaWhat != constMsgDelTopic {
		...
		default:
			// Reply with a 503 to the user.
			s.queueOut(ErrServiceUnavailableReply(msg, msg.Timestamp))
			globals.l.Sugar().Errorf("s.del: sub.meta channel full, topic ", msg.RcptTo, s.sid)
		}
		...
		default:
			// Reply with a 503 to the user.
			s.queueOut(ErrServiceUnavailableReply(msg, msg.Timestamp))
			globals.l.Sugar().Errorf("s.del: hub.unreg channel full", s.sid)
		}

	} else {
		// Must join the topic to delete messages or subscriptions.
		s.queueOut(ErrAttachFirst(msg, msg.Timestamp))
		globals.l.Sugar().Warnf("s.del: invalid Del action while unsubbed", msg.Del.What, s.sid)
	}
...
func (s *Session) note(msg *ClientComMessage) {
	...
	if sub := s.getSub(msg.RcptTo); sub != nil {
		...
		default:
			// Reply with a 503 to the user.
			s.queueOut(ErrServiceUnavailableReply(msg, msg.Timestamp))
			globals.l.Sugar().Errorf("s.note: sub.broacast channel full, topic ", msg.RcptTo, s.sid)
		}
		...
		default:
			// Reply with a 503 to the user.
			s.queueOut(ErrServiceUnavailableReply(msg, msg.Timestamp))
			globals.l.Sugar().Errorf("s.note: hub.route channel full", s.sid)
		}

	} else {
		s.queueOut(ErrAttachFirst(msg, msg.Timestamp))
		globals.l.Sugar().Warnf("s.note: note to invalid topic - must subscribe first", msg.Note.What, s.sid)
	}
}
...
func (s *Session) expandTopicName(msg *ClientComMessage) (string, *ServerComMessage) {
	if msg.Original == "" {
		globals.l.Sugar().Warnf("s.etn: empty topic name", s.sid)
	...
	if msg.Original == "me" {
		...
		if uid2.IsZero() {
			...
			globals.l.Sugar().Warnf("s.etn: failed to parse p2p topic name", s.sid)
			return "", ErrMalformed(msg.Id, msg.Original, msg.Timestamp)

		} else if uid2 == uid1 {
			// Use 'me' to access self-topic.
			globals.l.Sugar().Warnf("s.etn: invalid p2p self-subscription", s.sid)
