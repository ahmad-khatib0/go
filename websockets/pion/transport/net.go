...
type Net interface {
	...
	// The IP transports are "ip", "ip4", or "ip6" followed by a colon and a literal
	...
	// or a literal unspecified IP address, ListenPacket listens on all available
	// IP addresses of the local system except multicast IP addresses
	//
	// To only use ipv4, use network "udp4" or "ip4:proto". The address can use
	// a host name, but this is not recommended, because it will create a listener
	// for at most one of the host's IP addresses.
	//
	...
	// "udp", "udp4" (IPv4-only), "udp6"
	// (IPv6-only), "ip", "ip4" (IPv4-only), "ip6" (IPv6-only),
	...
	// It depends on each operating system how the operating system behaves
	// with a non-well known protocol number such as "0" or "255".
	//
	// Examples:
	//
	...
	// The network must be a UDP network name; see func Dial for details.
	//
	// If laddr is nil, a local address is automatically chosen. If the IP field
	// of raddr is nil or an unspecified IP address, the local system is assumed.
...
type UDPConn interface {
	...
	// The deadline applies to all future and pending I/O, not just the immediately following
	// call to Read or Write. After a deadline has been exceeded, the connection can be
	// refreshed by setting a deadline in the future.
	//
	// If the deadline is exceeded a call to Read or Write or to other I/O methods will
	// return an error that wraps os.ErrDeadlineExceeded. This can be tested using
	// errors.Is(err, os.ErrDeadlineExceeded). The error's Timeout method will return true,
	// but note that there are other possible errors for which the Timeout method will
	...
	// SetWriteDeadline sets the deadline for future Write calls and any currently-blocked
	// Write call. Even if write times out, it may return n > 0, indicating that some of
	// the data was successfully written. A zero value for t means Write will not time out.
	...
	// return an error after a fixed time limit; see SetDeadline and SetReadDeadline.
	Read(b []byte) (n int, err error)

	// ReadFrom reads a packet from the connection, copying the payload into p. It returns
	// the number of bytes copied into p and the return address that was on the packet.
	// It returns the number of bytes read (0 <= n <= len(p)) and any error encountered.
	//
	// Callers should always process the n > 0 bytes returned before considering the
	// error err. ReadFrom can be made to time out and return an error after a
	...
	//
	// The packages golang.org/x/net/ipv4 and golang.org/x/net/ipv6
	// can be used to manipulate IP-level socket options in oob.
	ReadMsgUDP(b, oob []byte) (n, oobn, flags int, addr *net.UDPAddr, err error)

	// Write writes data to the connection. Write can be made to time out and
	// return an error after a fixed time limit; see SetDeadline and SetWriteDeadline.
	Write(b []byte) (n int, err error)

	// WriteTo writes a packet with payload p to addr. WriteTo can be made to
	// time out and return an Error after a fixed time limit; see SetDeadline and
	// SetWriteDeadline. On packet-oriented connections, write timeouts are rare.
	WriteTo(p []byte, addr net.Addr) (n int, err error)

	// WriteToUDP acts like WriteTo but takes a UDPAddr.
	WriteToUDP(b []byte, addr *net.UDPAddr) (int, error)

	// WriteMsgUDP writes a message to addr via c if c isn't connected, or to c's remote
	// address if c is connected (in which case addr must be nil). The payload is copied
	// from b and the associated out-of-band data is copied from oob. It returns the
