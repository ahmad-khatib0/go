...
func TestNetTest(t *testing.T) {
	nettest.TestConn(t, func() (net.Conn, net.Conn, func(), error) {
		...
		return &closePropagator{caConn, cbConn}, &closePropagator{cbConn, caConn}, func() {
			_ = ca.Close()
			_ = cb.Close()
		}, nil
	})
...
func TestPipe(t *testing.T) { //nolint:cyclop
	...
	// In this test, ca is closed, but cb is still open. The cb.Read call blocks because:
	// 1- There is no data in the c.rCh channel.
	// 2- The connection is not explicitly closed yet (it is closed later in the test with
	//    cb.Close()). So case <-readDone won't be executed here because the closing dose not
	//    occur for the channel, because it blocked inside the cb.Read() in the for loop
	select {
	case <-readDone:
