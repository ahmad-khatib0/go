...
func TestBridge(t *testing.T) { //nolint:gocyclo,cyclop,maintidx
	...
	t.Run("normal", func(t *testing.T) {
		...
		if conn0.LocalAddr().Network() != "udp" {
			t.Error("conn0 local addr name should be udp")
		}

		if conn1.LocalAddr().Network() != "udp" {
			t.Error("conn1 local addr name should be udp")
		}
		...
		testFrom := func(t *testing.T, fromID int) {
			...
			go func() {
				...
				for {
					...
					readRes <- AsyncResult{n: nInner, err: nil, msg: string(buf)}
				}
			}()
...
func TestNetTest(t *testing.T) {
	nettest.TestConn(t, func() (c1 net.Conn, c2 net.Conn, stop func(), err error) {
		...
		return &closePropagator{conn0, conn1}, &closePropagator{conn1, conn0},
			func() {
				// RacyRead test leave receive buffer filled. As net.Conn.Read() should
				// return received data even after Close()-ed, queue must be cleared explicitly.
				br.clear()
				_ = conn0.Close()
				_ = conn1.Close()
