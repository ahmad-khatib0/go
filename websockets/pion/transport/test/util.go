...
// TimeOut is used to panic if a test takes to long. It will print the current
//
// goroutines and panic. It is meant as an aid in debugging deadlocks.
...
func getRoutines() []string {
	buf := make([]byte, 2<<20)
	// 	This line resizes the slice buf to the actual number of bytes written by runtime.Stack:
	//  - runtime.Stack(buf, true) returns the number of bytes written.
	//  - buf[:n] creates a new slice that only includes the first n bytes of buf.
	// This ensures that buf only contains the relevant stack trace data, without extra unused space.
...
func CheckRoutinesStrict(tb testing.TB) func() {
	...
	return func() {
		...
		// goroutines that really aren't doing anything but haven't yet completed.
