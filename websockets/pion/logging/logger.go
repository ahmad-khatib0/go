...
// Debug emits the pre-formatted message if the logger is at or below LogLevelDebug.
...
// Info emits the pre-formatted message if the logger is at or below LogLevelInfo.
...
// Warn emits the pre-formatted message if the logger is at or below LogLevelWarn.
...
// Error emits the pre-formatted message if the logger is at or below LogLevelError.
...
func NewDefaultLeveledLoggerForScope(scope string, level LogLevel, writer io.Writer) *DefaultLeveledLogger {
	...
	logger := &DefaultLeveledLogger{writer: &loggerWriter{output: writer}, level: level}
	return logger.
...
// NewLogger returns a configured LeveledLogger for the given, args scope.
