...
import (
	...
	metadatatest "github.com/ahmad-khatib0/go/microservice/movies/metadata/pkg/testutil"
	movietest "github.com/ahmad-khatib0/go/microservice/movies/movie/pkg/testutil"
	"github.com/ahmad-khatib0/go/microservice/movies/pkg/discovery"
	"github.com/ahmad-khatib0/go/microservice/movies/pkg/discovery/memory"
	ratingtest "github.com/ahmad-khatib0/go/microservice/movies/rating/pkg/testutil"
...
func main() {
	...
	wantMovieDetails := &gen.MovieDetails{Metadata: m}
...
func startMetadataService(ctx context.Context, registry discovery.Registry) *grpc.Server {
	...
	// we call the srv.Serve function inside a goroutine — this way, it doesn’t block
	// the execution and allows us to immediately return from the function.
	go func() {
	...
	if err := registry.Registr(ctx, id, metadataServiceName, metadataServiceAddr); err != nil {
		panic(err)
	}
	return srv
}
...
func startRatingService(ctx context.Context, registry discovery.Registry) *grpc.Server {
	...
	if err := registry.Registr(ctx, id, ratingServiceName, ratingServiceAddr); err != nil {
		panic(err)
	}
...
func startMovieService(ctx context.Context, registry discovery.Registry) *grpc.Server {
	...
	if err := registry.Registr(ctx, id, movieServiceName, movieServiceAddr); err != nil {
		panic(err)
	}
	return srv
}
