...
func setupMember(t *testing.T, members []*Membership) ([]*Membership, *handler) {
	...
	c := Config{
		NodeName: fmt.Sprintf("%d", id), // the member’s length as the node name so the names are unique
		BindAddr: addr,
		Tags:     tags,
	}
	h := &handler{}
	...
	//  ┌──────────────────────────────────────────────────────────────┐
	//    The member’s length also tells us whether this member is the
	//    cluster’s initial member or we have a cluster to join.
	//  └──────────────────────────────────────────────────────────────┘
	if len(members) == 0 {
