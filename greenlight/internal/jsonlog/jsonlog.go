...
// Level the severity level for a log entry.
type Level int8
...
// Logger holds the output destination that the log entries will be written to, the minimum severity
// level that log entries will be written for, plus a mutex for coordinating the writes.
type Logger struct {
	out      io.Writer
	minLevel Level
	mu       sync.Mutex
...
// Write() it satisfies the io.Writer interface. writes a log entry at the ERROR level with no additional properties.
